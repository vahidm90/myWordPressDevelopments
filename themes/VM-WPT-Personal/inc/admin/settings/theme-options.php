...
function vm_theme_options() {
	...
	/*
	 * Main theme options (slug:vm-theme-options)
	 */

	add_settings_section(
		'vm-theme-options-front-page-section',
		__( 'Front-page options', VM_TD ),
	...
	$num_args = array(
		...
		'field_title'        => _x( 'Tiers (allowed: 1-99)', 'Tiers count field text', VM_TD ),
		'field_markup'       => 'vm_number_option_field_markup',
		'section_id'         => 'vm-theme-options-front-page-section',
		'option_type'        => 'number',
		'option_description' => __( 'Number of front-page tiers', VM_TD ),
		'option_default'     => 1,
	...
	add_settings_section(
		"vm-front-page-tiers-common-options-section",
		_x( 'Global Settings', 'Setting section title', VM_TD ),
	...
	$text_args = array(
		...
		'option_name'        => 'vm_theme_options_front_page_tiers_generic_common_classes',
		'option_group'       => 'vm_theme_options',
		'field_title'        => __( 'Class(es) shared among all tiers (separated by space)', VM_TD ),
		'section_id'         => 'vm-theme-options-front-page-section',
		'option_description' => __( 'Class(es) to use for all front-page tiers', VM_TD ),
	);
	...
	$text_args = array(
		'field_id'           => 'vm-front-page-tiers-common-classes',
		'page_slug'          => 'vm-front-page-tiers-options',
		'option_name'        => 'vm_theme_options_front_page_tiers_generic_template',
		'option_group'       => 'vm_theme_options',
		'field_title'        => __( 'Class(es) shared among all tiers (separated by space)', VM_TD ),
		'section_id'         => 'vm-theme-options-front-page-section',
		'option_description' => __( 'Class(es) to use for all front-page tiers', VM_TD ),
	...
	for ( $i = 1; (int) get_option( 'vm_theme_options_front_page_tiers_count' ) >= $i; $i ++ ) :
		...
		add_settings_section(
			...
			sprintf( _x( 'Tier %d', 'Setting section title; %d: Tier number', VM_TD ), $i ),
			'vm_front_page_tiers_options_section_markup',
			'vm-front-page-tiers-options'
		);
//TODO: Create separate fields for each piece of data.
		...
		$text_args = array(
			...
			'field_title'        => _x( 'Template', 'Tier option field text', VM_TD ),
			'section_id'         => "vm-front-page-tiers-options-tier-$i-section",
			'option_description' => sprintf(
				_x( 'Front-page tier %d template file', 'Tier option description; %d: Tier number', VM_TD ),
				$i
			),
		);
		...
		$text_args['field_title']        = _x( 'Exclusive class(es)', 'Tier option field text', VM_TD );
		$text_args['option_name']        = "vm_theme_options_front_page_tier_{$i}_classes";
		$text_args['option_description'] = sprintf(
			_x( 'Front-page tier %d exclusive class(es)', 'Tier option description; %d: Tier number', VM_TD ),
			$i
		);
		...
		$text_args['field_title']        = _x( 'Excluded common class(es)', 'Tier option field text', VM_TD );
		$text_args['option_name']        = "vm_theme_options_front_page_tier_{$i}_stripped_classes";
		$text_args['option_description'] = sprintf(
			_x( 'Front-page tier %d excluded class(es)', 'Tier option description; %d: Tier number', VM_TD ),
			$i
		);
		...
		$bool_args = array(
			...
			'option_group'       => 'vm_front_page_tiers_options',
			'field_title'        => _x( 'Include in tiers navigation menu', 'Tier option field text', VM_TD ),
			'field_markup'       => 'vm_checkbox_option_field_markup',
			'section_id'         => "vm-front-page-tiers-options-tier-$i-section",
			'option_type'        => 'boolean',
			'option_description' => sprintf(
				_x( 'Front-page tier %d menu inclusion', 'Tier option description; %d: Tier number', VM_TD ),
				$i
			),
		...
		$text_args['field_title']        = _x( 'Menu title', 'Tier option field text', VM_TD );
		...
		$text_args['option_description'] = sprintf(
			_x( 'Front-page tier %d menu title', 'Tier option description; %d: Tier number', VM_TD ),
			$i
		);
		...
		$hide_args = array(
			...
			'option_group'       => 'vm_front_page_tiers_options',
			'field_title'        => _x( 'Background', 'Tier option field text', VM_TD ),
			'field_markup'       => 'vm_front_page_tier_background_image_option_field_markup',
			'section_id'         => "vm-front-page-tiers-options-tier-$i-section",
			'option_type'        => 'number',
			'option_description' => sprintf(
				_x( 'Front-page iter %d background', 'Tier option description; %d: Tier number', VM_TD ),
				$i
			),
			'option_default'     => 0,
		);
